...
apiClient.interceptors.response.use(
  ...
  (error) => {
    if (error.response) {
      ...
      return Promise.reject(Error('An unexpected error occurred.  Invalid response from the server.'))
    } else if (error.request) {
      return Promise.reject(Error('An unexpected error occurred.  No response received.'))
    } else {
      return Promise.reject(Error('An unexpected error occurred.  Failed to process the request.'))
    }
  }
)
